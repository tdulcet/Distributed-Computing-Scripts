...
VERSION = 19
primenet_v5_bargs = OrderedDict((("px", "GIMPS"), ("v", PRIMENET_TRANSACTION_API_VERSION)))
...
def get_assignment(progress):
	...
	if time_left is None or time_left <= options.days_work*24*3600:
		...
		num_cache += 1
		debug_print("Time_left is {0} and smaller than limit ({1}), so num_cache is increased by one to {2}".format(time_left, options.days_work*24*3600, num_cache))
...
def mersenne_find(line, complete=True):
	...
	return re.search("[Pp]rogram|CUDALucas", line)
...
def register_instance(guid):
	...
	args["a"] = platform.system() + ('64' if platform.machine().endswith('64') else '') + ",Mlucas,v" + str(VERSION)	#
...
def get_progress_assignment(task):
	...
	if not found:
		...
		debug_print("ERROR: Unable to extract valid PrimeNet assignment ID from entry in " + workfile + ": " + str(task[0]), file=sys.stderr)
		return
...
def send_progress(assignment_id, is_prp, percent, time_left, retry_count=0):
	...
	if guid is None:
		...
		debug_print("Run primenet.py with the --register option", file=sys.stderr)
		return
	...
	if result is None:
		...
		if rc == primenet_api.ERROR_OK:
			...
			if rc is primenet_api.ERROR_UNREGISTERED_CPU:
				...
				debug_print("INVALID PARAMETER: this is a bug in the script, please create an issue: https://github.com/tdulcet/Distributed-Computing-Scripts/issues", file=sys.stderr)
				return False
			else:
...
parser = OptionParser(version="%prog 1.0", description= \
	u"""This program will automatically get assignments, report assignment results and optionally progress to PrimeNet for both the CUDALucas and Mlucas GIMPS programs. It also saves its configuration to a “local.ini” file, so it is only necessary to give most of the arguments the first time it is run.
...
parser.add_option("-w", "--workdir", dest="workdir", default=".", help=u"Working directory with “worktodo.ini” and “results.txt” from the GIMPS program, and “local.ini” from this program, Default: %default (current directory)")
parser.add_option("-i", "--workfile", dest="workfile", default="worktodo.ini", help=u"WorkFile filename, Default: “%default”")
parser.add_option("--resultsfile", dest="resultsfile", default="results.txt", help=u"ResultsFile filename, Default: “%default”")

# all other options are saved to local.ini (except --register)
parser.add_option("-u", "--username", dest="username", help="GIMPS/PrimeNet User ID. Create a GIMPS/PrimeNet account: https://www.mersenne.org/update/. If you do not want a PrimeNet account, you can use ANONYMOUS.")
parser.add_option("-p", "--password", dest="password", help="GIMPS/PrimeNet Password. Only provide if you want to do manual testing and not report the progress (not recommend). This was the default behavior for old versions of this script.")

# -t is reserved for timeout, instead use -T for assignment-type preference:
parser.add_option("-T", "--worktype", dest="worktype", default="100", help= \
	"""Type of work, Default: %default,
...
parser.add_option("-g", "--gpu", action="store_true", dest="gpu", default=False, help="Get assignments for a GPU (CUDALucas) instead of the CPU (Mlucas)")
parser.add_option("-c", "--cpu_num", dest="cpu", type="int", default=0, help="CPU core or GPU number to get assignments for, Default: %default")
parser.add_option("-n", "--num_cache", dest="num_cache", type="int", default=0, help="Number of assignments to cache, Default: %default")
parser.add_option("-L", "--days_work", dest="days_work", type="int", default=3, help="Days of work to queue, Default: %default days. Add one to num_cache when the time left for the current assignment is less then this number of days.")

parser.add_option("-t", "--timeout", dest="timeout", type="int", default=60*60*6, help="Seconds to wait between network updates, Default: %default seconds (6 hours). Use 0 for a single update without looping.")
...
group.add_option("-r", "--register", action="store_true", dest="register", default=False, help="Register this GIMPS program with PrimeNet")
group.add_option("-H", "--hostname", dest="hostname", default=platform.node()[:20], help="Computer name, Default: %default")
# TODO: add detection for most parameter, including automatic change of the hardware
group.add_option("--cpu_model", dest="cpu_model", default="cpu.unknown", help="Processor (CPU) model, Default: %default")
group.add_option("--features", dest="features", default="", help="CPU features, Default: '%default'")
group.add_option("--frequency", dest="frequency", type="int", default=1000, help="CPU frequency (MHz), Default: %default MHz")
group.add_option("-m", "--memory", dest="memory", type="int", default=0, help="Total memory (RAM) (MiB), Default: %default MiB")
group.add_option("--L1", dest="L1", type="int", default=8, help="L1 Cache size (KiB), Default: %default KiB")
group.add_option("--L2", dest="L2", type="int", default=512, help="L2 Cache size (KiB), Default: %default KiB")
group.add_option("--np", dest="np", type="int", default=1, help="Number of CPU Cores, Default: %default")
group.add_option("--hp", dest="hp", type="int", default=0, help="Number of CPU threads per core (0 is unknown), Default: %default")
...
if options.password is None and guid is None:
	parser.error("Password must be given or run with the --register option")
...
while True:
	...
	if got > 0:
		debug_print("Redo progress update to update the just obtained assignment(s)")
