...
def primenet_fetch(num_to_get):
    ...
    try:
        openurl = primenet_baseurl + "manual_result/?"
        debug_print("Fetching work via URL = "+openurl + assignment) # TODO -- this doesn't urlencode the assignment as we are trying to get rid of the urlencode...
        r = s.post(url, data={"data": openurl})
...
def create_new_guid():
    guid = hex(random.getrandbits(128))
    if guid[:2] == '0x':
